...
unsigned char BUFF_SET[BUF_SIZE] = {FLAG, A1, C2, A1 ^ C2, FLAG};
unsigned char BUFF_UA[BUF_SIZE]  = {FLAG, A1, C1, A1 ^ C1, FLAG};
...
int stateMachine(unsigned char byte, unsigned char control)
{
    ...
    switch (state)
    {
        ...
        case A_RCV:
            if (byte == control) state = C_RCV;
            else if (byte == FLAG) state = FLAG_RCV;
            else state = START;
            break;

        case C_RCV:
            if (byte == (A1 ^ control)) state = BCC_OK;
            else if (byte == FLAG) state = FLAG_RCV;
            else state = START;
            break;
